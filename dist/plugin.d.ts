...
export type ProvideSchema = (esbuildHash: string) => T.Effect<OT.HasTracer & HasConsole, SourceProvideSchemaError, SchemaDef>;
export type FetchData = (_: {
    schemaDef: SchemaDef;
    verbose: boolean;
    skipCachePersistence?: boolean;
}) => S.Stream<OT.HasTracer & HasClock & HasCwd & HasConsole & fs.HasFs, never, E.Either<SourceFetchDataError | SourceProvideSchemaError, DataCache.Cache>>;
export type MakeSourcePlugin<TArgs extends PartialArgs> = (_: TArgs | ThunkWithSourceKey<TArgs> | ThunkWithSourceKey<Promise<TArgs>>) => (sourceKey: string | undefined) => Promise<SourcePlugin>;
...
export declare const processArgs: <TArgs extends PartialArgs>(argsOrArgsThunk: TArgs | ThunkWithSourceKey<TArgs> | ThunkWithSourceKey<Promise<TArgs>>, sourceKey: string | undefined) => Promise<{
    extensions: PluginExtensions;
    options: PluginOptions;
    restArgs: Omit<TArgs, "extensions" | "fieldOptions" | "markdown" | "mdx" | "date" | "disableImportAliasWarning" | "experimental" | "onSuccess">;
}>;
